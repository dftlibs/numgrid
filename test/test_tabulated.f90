program test
   ...
   open(unit=io_unit, file='../test/reference_grid/dyall.v3z.txt', access='sequential', action='read')

   context = numgrid_new_atom_grid_tabulated(radial_precision,       &
                                             min_num_angular_points, &
                                             max_num_angular_points, &
                                             proton_charges(1))

   num_points = numgrid_get_num_grid_points(context)
   if (num_points /= 19310) stop 1
   ...
   if (num_radial_points /= 127) stop 1
